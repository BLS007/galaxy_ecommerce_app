...
// Importing the Sports Nutrition Data.
import SportsNutritionData from '../../Data/HealthHousehold/SportsNutritionData';

const SportsNutrition = () => {

    {/* Utilizing the Sports Nutrition Data, we can implement one card entry below to make the code on this page less redundant. */}
    const cardItem = (item) => {
        return (
            <div class="card my-5 py-4 bg-dark text-light border border-light" key={item.id} style={{ width: "18rem" }}>
                ...
                <div class="card-body text-center">
                    ...
                    <NavLink to={`/sportsnutrition/${item.id}`} className="btn btn-outline-warning">Buy Now</NavLink>
                </div>
            </div>
        );
    };

    {/* Returning the title, borders sitting on the top and bottom of the title, followed by the individual card items from the Sports Nutrition Data page. 
        ...
        <div className="position-relative bg-dark">
            ...
            <div className="container">
                <div className="row justify-content-around">
                    {SportsNutritionData.map(cardItem)}
                </div>
            </div>
        </div>
    );
