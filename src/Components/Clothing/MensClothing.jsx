...
// Importing the Mens Clothing Data.
import MensClothingData from '../../Data/Clothing/MensClothingData';

const MensClothing = () => {

    {/* Utilizing the Mens Clothing Data, we can implement one card entry below to make the code on this page less redundant. */}
    const cardItem = (item) => {
        return (
            <div class="card my-5 py-4 bg-dark text-light border border-light" key={item.id} style={{ width: "18rem" }}>
                ...
                <div class="card-body text-center">
                    ...
                    <NavLink to={`/mensclothing/${item.id}`} className="btn btn-outline-warning">Buy Now</NavLink>
                </div>
            </div>
        );
    }

    {/* Returning the title, borders sitting on the top and bottom of the title, followed by the individual card items from the Mens Clothing Data page. 
