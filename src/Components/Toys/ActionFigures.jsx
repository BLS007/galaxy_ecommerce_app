...
// Importing the Action Figures Data.
import ActionFiguresData from '../../Data/Toys/ActionFiguresData';

const ActionFigures = () => {

    {/* Utilizing the Action Figures Data, we can implement one card entry below to make the code on this page less redundant. */}
    const cardItem = (item) => {
        return (
            <div class="card my-5 py-4 bg-dark text-light border border-light" key={item.id} style={{ width: "18rem" }}>
                ...
                <div class="card-body text-center">
                    ...
                    <NavLink to={`/actionfigures/${item.id}`} className="btn btn-outline-warning">Buy Now</NavLink>
                </div>
            </div>
        );
    };

    {/* Returning the title, borders sitting on the top and bottom of the title, followed by the individual card items from the Action Figures Data page. 
        ...
        <div className="position-relative bg-dark">
            ...
            <div className="container">
                <div className="row justify-content-around">
                    {ActionFiguresData.map(cardItem)}
                </div>
            </div>
        </div>
    );
